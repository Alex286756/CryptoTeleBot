...
def start_message(message):
    ...
    """
    global target, source, code, input_text, key, need_key_file, key_filename, code_type
    target = None
    source = None
    input_text = None
    key = None
...
def help_message(message):
    ...
    chat_id = message.chat.id
    first_name = message.chat.first_name
    bot.send_message(chat_id, "Для старта программы напишите /start")
...
@bot.message_handler(content_types=['document'], func=lambda message: 'png' in message.document.mime_type)
