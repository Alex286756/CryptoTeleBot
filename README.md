...
typing - определения типов Optional и Union  
dotenv - для работы с переменными окружения  

Запуск бота:

Для запуска бота необходимо иметь переменную окружения TELEGRAM_TOKEN, содержащую токен для управления ботом.
Например, можно создать в корне бота файл '.env' и в него добавить строку с токеном:  

```
TELEGRAM_TOKEN=<token>
```

После этого запуск бота осуществляется командой:

```
python main.py
```

Работа пользователя с ботом начинается с команды '/start'.  
По команде '/help' выдается небольшая справка по работе бота.  
